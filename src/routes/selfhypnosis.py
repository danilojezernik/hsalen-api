...
Routes:
1. GET self-hypnosis to insert log
"""
...
# GET ALL BLOG
@router.get("/", operation_id="get_self_hypnosis")
async def get_all(request: Request):
    ...
    try:
        ...
        log_entry = Logging(
            ...
            content=f'Request made to: SAMO-HIPNOZA - {route_method} ',
            datum_vnosa=datetime.datetime.now()
        )
        db.log.backend_logs.insert_one(log_entry.dict(by_alias=True))

        return {'message': 'Samo-hipnoza initialized'}
